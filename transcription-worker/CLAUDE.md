...
- **CI/CD:** Push to `main` (any change under `transcription-worker/` or `gpu-worker/`) runs root `.github/workflows/worker.yml` via `deploy.yml` (after the API deploy, so schema changes land first) — it builds/pushes the image and registers a new ECS task-definition revision; there is no service to roll, since the API launches the worker per job with `RunTask`. The next launch uses the new revision; cold starts pull only the layers that differ from the baked image (seconds for a code-only change, ~5 min if the base or model layers changed) until the GPU AMI is re-baked (`scripts/deploy/build-gpu-ami.sh`, see `docs/runbooks/deploy.md`)
...
- **Base image:** `nvidia/cuda:12.8.1-cudnn-runtime-ubuntu24.04` (Python 3.12); requires `>=3.12`. Every Python package is pinned by `constraints.txt` (a `pip freeze` of the acceptance-tested image, passed with `-c` on each `pip install`) after an unpinned rebuild picked up `speechbrain` 1.1.1 on 2026-08-28; `tests/test_constraints.py` checks the Dockerfile and pyproject against it. To upgrade a package, edit the pin (or rebuild without `-c`, freeze, paste the result)
