...
def get_transcription_service(db: AsyncSession = Depends(get_db)) -> TranscriptionService | LocalTranscriptionService:
    ...
    if settings.use_mock_transcription:
        ...
        return LocalTranscriptionService(repo, storage, _mock_sqs, settings)
    storage = AudioStorageService(settings)
    sqs = SQSPublisher(settings.transcribe_sqs_queue_url, settings.aws_region)
    return TranscriptionService(repo, storage, sqs, settings)
